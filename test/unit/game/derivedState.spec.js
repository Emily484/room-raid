...
describe('derivedState helpers', () => {
  it('weighted average and clamp produce expected floorClutter', () => {
    const s = { clothingOnFloor: 100, cardboard: 0, floorTrash: 0, miscellaneousFloorItems: 0 };
  ...
  it('exposed floor clamps below 0/above 100', () => {
    expect(calculateExposedFloor({ floorObstruction: -50 })).toBe(100);
    expect(calculateExposedFloor({ floorObstruction: 150 })).toBe(0);
  });

  it('floor readiness uses exposed floor and floorClutter', () => {
    const s = { floorObstruction: 0, clothingOnFloor: 0, cardboard: 0, floorTrash: 0, miscellaneousFloorItems: 0 };
